...
public class AuditNewGanttChart extends JPanel implements DataSourceListener {
  ...
  private DjtDiagram createAllSeries() {
    DjtDiagram diagramA = new DjtDiagram(res.getString("aaDefinition.label.audit"));
    diagramA.setFont(new Font("Dialog", 0, 10));

    AuditStatedDao auditStatedDao = new AuditStatedDao();

    for (int i = 0; i < dataVector.size(); i++) {
      auditStatedDao = (AuditStatedDao)dataVector.elementAt(i);
      ...
      try {
	startDate = new Date(auditStatedDao.auditDate.getValue().getTime());
	endDate = new Date(auditStatedDao.auditEndDate.getValue().getTime());
	startDateAct = new Date(auditStatedDao.auditRealStartDate.getValue().getTime());
	endDateAct = new Date(auditStatedDao.auditRealEndDate.getValue().getTime());
      }
      catch (Exception ex1) {
      }
      ...
      DjtDiagram diagramA1 = new DjtDiagram(res.getString("auditDefinition.label.auditPlan") + ": " +
					    auditStatedDao.auditPlanID.getValue() +" "+res.getString("auditDefinition.label.auditDate")+": "+dateFormat.format(auditStatedDao.auditDate.getValue()));
	...
	activity.setToolTipText(res.getString("auditGantt.label.planned") + (i + 1) + "");
	...
	entry.setToolTipText(res.getString("auditGantt.label.planned") + (i + 1) + "");
	...
	entry.setToolTipText(res.getString("auditGantt.label.actual") + (i + 1) + "");
	diagramA1.addEntry(entry);
      }
	...
	activity.setToolTipText(res.getString("auditGantt.label.actual") + (i + 1) + "");
	diagramA1.addEntry(activity);
      }
  ...
  private DjtSheet createSheet() {
    ...
    DjtSheet sheet = new DjtSheet(res.getString("auditGantt.label.planning"));
    ...
    m.addColor(res.getString("auditGantt.label.actual"), Color.red);
    m.addColor(res.getString("auditGantt.label.planned"), Color.blue);
