 ...
 * @version 0.1
 */
public class AuditGanttDefinition extends JPanel {
  ...
  AuditGanttDataSource agDS = new AuditGanttDataSource();
  ...
  private void initialize() {
	...
	agDS.setNullValues(false);
	  ...
	  if ((jtfldAuditEndDate.getSelectedDate(true).before(jtfldAuditDate.getSelectedDate(true))) || (jtfldAuditEndDate.getSelectedDate(true).equals(jtfldAuditDate.getSelectedDate(true)))) {
	    ...
	    try {
	      agDS.setParameters(new String(MenuSingleton.isSupplier() ? MenuSingleton.getSelectedEnterprise() : MenuSingleton.getEnterprise()),
    ...
    add(new AuditNewGanttChart(agDS, jbtnUpdate), BorderLayout.CENTER);
  }
}
