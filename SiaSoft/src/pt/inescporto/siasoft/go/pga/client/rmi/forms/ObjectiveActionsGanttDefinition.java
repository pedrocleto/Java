 ...
 * @version 0.1
 */
public class ObjectiveActionsGanttDefinition extends JPanel {

  static ResourceBundle res = ResourceBundle.getBundle("pt.inescporto.siasoft.go.pga.client.rmi.forms.FormResources");

  ObjectiveActionsGanttDataSource oagDS = new ObjectiveActionsGanttDataSource();
  ...
  public ObjectiveActionsGanttDefinition() {
    initialize();
  }
  ...
  private void initialize() {
	...
	oagDS.setNullValues(false);
	    ...
	    try {
	      oagDS.setParameters(new String(MenuSingleton.isSupplier() ? MenuSingleton.getSelectedEnterprise() : MenuSingleton.getEnterprise()),
    ...
    add(new ObjectiveActionsNewGanttChart(oagDS, jbtnUpdate), BorderLayout.CENTER);
  }
}
