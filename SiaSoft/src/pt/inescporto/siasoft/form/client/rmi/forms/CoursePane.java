 ...
 * @version 0.1
 */
public class CoursePane extends JPanel {

  DataSource datasource = null;
  FW_ComponentListener fwCListener = null;
  FW_NavBarDetail navBarDetail = new FW_NavBarDetail();
  FW_JTable fwCourseList = null;
  JScrollPane jsp = new JScrollPane();
  static ResourceBundle res = ResourceBundle.getBundle("pt.inescporto.siasoft.form.client.rmi.forms.FormResources");

  public CoursePane(DataSource datasource, FW_ComponentListener fwCListener) {
    this.datasource = datasource;
    this.fwCListener = fwCListener;
    initialize();
    //table listener registration
    navBarDetail.setFW_ComponentListener(fwCourseList);
    navBarDetail.setActionListener(fwCourseList);
    fwCListener.addFWComponentListener(fwCourseList);
  }
  ...
  private void initialize() {
    ...
    colManager.addColumnNode(new FW_ColumnNode(6,
					       res.getString("coursePane.label.user"),
					       "fkUserID",
					       new TmplLookupRenderer("pt.inescporto.siasoft.comun.ejb.session.User"),
					       cbeUser));
    try {
      fwCourseList = new FW_JTable(datasource.getDataSourceByName("Course"), null, colManager);
    }
    catch (DataSourceException ex) {
      ex.printStackTrace();
    }

    fwCourseList.setAsMaster(false);
    ...
    jsp.getViewport().add(fwCourseList);
